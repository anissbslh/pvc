...
function tsp() {
  ...
  console.log("Nearest neighbour", o3.result, "time", t3 - t2);
...
var tak = [];
  var ek =  {}
  for (var i =1; i< o3.nodes.length; i++) {
    ek.from = o3.nodes[i];
    if (i!=o3.nodes.length-1) {
      ek.to = o3.nodes[i+1];
      tak.push(ek)
    } else {
      ek.to = 0;
  ...
  edges.forEach((element) => {
    ...
    } else if (containsObject(el, tak) || containsObject(el2, tak)) {
      edges.update({ id: element.id, color: { color: "#000000" } });
    } else {
      edges.update({ id: element.id, color: { color: "blue" } });
    }
  });

  document.getElementById("exhaustive-time").innerHTML =
    (t1 - t0).toFixed(5) + " ms";
  document.getElementById("exhaustive").innerHTML = o.min_path;
  document.getElementById("vorace-time").innerHTML =
    (t2 - t1).toFixed(5) + " ms";
  document.getElementById("vorace").innerHTML = o2;
  document.getElementById("nn-time").innerHTML = (t3 - t2).toFixed(5) + " ms";
  document.getElementById("nn").innerHTML = o3.result;
